...
# Source folders
src='./wirtschaftstag_webservice_v2'
bin='./target'
...
# Docker image names
DockerJPAImageName='springdatajpav2'
...
echo;echo "$cnt/$lim Starting the docker containers..."; cnt=$[$cnt+1]
	docker-compose up -d
# This is for some reason just broken now
# if [ $? -ge 1 ]; then echo "An error occured. Aborting."; exit; fi
...
echo;printf "$cnt/$lim Opening firefox..."; cnt=$[$cnt+1]
	firefox "$WebserverProtocol://$WebserverHostname:$WebserverPort/static/index.html" &	# & so it is non blocking
if [ $? -ge 1 ]; then echo "An error occured. Aborting."; exit; fi
